			...
			if (grabbed == 0) {
				...
				while (grabbed) {
					...
					if (event.type == KeyPress || event.type == KeyRelease) {
						if (event.type == KeyRelease && ((event.xkey.keycode == tabMod1Key) || (event.xkey.keycode == tabMod2Key))) { /* if super key is released break cycle */
							break;
...
tile(Monitor *m)
{
   ...
   if (n > 1)
	...
	for(i = 0, my = ty = actgappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			h = (m->wh - my) * (c->cfact / mfacts) - actgappx;
			resize(c, m->rmaster ? m->wx + m->ww - mw + actgappx : m->wx + actgappx, m->wy + my, mw - (2*c->bw) - actgappx*(5-ns)/2, h - (2*c->bw), 0);
			if (my + HEIGHT(c) < m->wh)
				my += HEIGHT(c) + actgappx;
     mfacts -= c->cfact;
		} else {
			h = (m->wh - ty) * (c->cfact / sfacts) - actgappx;
			resize(c, m->rmaster ? m->wx + actgappx/ns: m->wx + mw + actgappx/ns, m->wy + ty,
			       m->ww - mw - (2*c->bw) - actgappx*(5-ns)/2, h - (2*c->bw), 0);
			if (ty + HEIGHT(c) < m->wh)
				ty += HEIGHT(c) + actgappx;
     sfacts -= c->cfact;
		}
}
